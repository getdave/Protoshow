...
var protoShow = Class.create({
	initialize: function(element,options) {
		...
		this.captionsElement	=	this.options.captionsElement;
	...
	updateNav: function(active,next) {
		if (this.protoNav != undefined) {
			this.protoNav[active].up('li').removeClassName('current-slide');
			this.protoNav[next].up('li').addClassName('current-slide');
		}
	},
	
	updateControls: function(status) {
		if (this.protoControls != undefined ) {
			...
			if (status) {
				...
				startStop.update(this.stopText).writeAttribute('title','Pause the slide show').up('li').addClassName('pause').removeClassName('play');
			} else {			
				// The show has been stopped so update the button to "Play"
				startStop.update(this.playText).writeAttribute('title','Play the slide show').up('li').addClassName('play').removeClassName('pause');
			}
		}
	},
	...
	createNavigation: function() {
		...
		if (this.element.select('.proto-navigation li a').length) {
			this.protoNav	= 	this.element.select('.proto-navigation li a');		// Select the navigation anchors <a>
			this.protoNav[this.slideID].up('li').addClassName('current-slide');		// Ensure the initial Nav item has "current-slide" class
	...
	createControls: function() {
		...
		if (this.element.down('.proto-controls') != undefined ) {
			this.protoControls	=  this.element.down('.proto-controls');    // Stop/Forward/Back buttons
			// Adds observers for Slide Show controls  
			var startStop    	=	this.protoControls.down('.start-stop a');
			var forward      	=	this.protoControls.down('.forward a');
			var backward     	=	this.protoControls.down('.backward a');
	...
	createCaptions: function() {
		if (this.captions) {
			if ($(this.captionsElement) != "undefined") {
				...
				this.captionElement		=	captionEle;
			}			
	...
	updateCaptions: function(next) {		
		if (this.captions) {
			...
			if (nextCaption.replace(/^\s*|\s*$/g,'').length) {		// check that the attribute has some content (not just spaces)
				if (!this.captionElement.visible()) {
					this.captionElement.show();
				}
				this.captionElement.down('p').update(nextCaption);	
			} else {
...
document.observe("dom:loaded", function() {
	var show1 = new protoShow('myshow1');
