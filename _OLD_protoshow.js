...
var protoShow = Class.create({
	...
	setupTimer: function() {	
		...
		this.timerSegments = 5;
		this.calculateTimerSpeed();	// set this.timerSpeed property
			
		
		/* To ensure speed per segment is not too fast to be animated on Canvas */
		/* We increment the number of timerSegments until the this.timerSpeed value is above 25ms */
		while(this.timerSpeed<25) { 
			this.timerSegments+=5;
			this.calculateTimerSpeed();
	...
	calculateTimerSpeed: function() {		
		this.timerTotalDegSegs 	= 360/this.timerSegments; // total number of degree segments needed to make up 360deg
		this.timerSpeed 		= this.interval/this.timerTotalDegSegs; // returns time required per arc degree/segment to be draw		
	},
	
	
	startTimer: function() {			
		//console.log("startTimer: Timer Started");
		/*this.slideTimer.appear({
			duration: 0.2
		});*/
		this.drawArc(0,360,'rgba(255,255,255,0.3)');
	
		this.runTimer(this.timerSegments);
			//this.slideTimer.appear();
	},
	
	stopTimer: function() {
		clearTimeout(this.timerClock);
		/*this.slideTimer.fade({
	...
	runTimer: function(endAngle) {
		if ((!this.animating) && endAngle <= 360) { // check a slide isn't animating and we're not above 360deg
			if(endAngle == 360) {
				console.log("Drawing arch " + endAngle);
			}
				/*if(endAngle==270) {
					var d = (this.timerSpeed*(90/this.timerSegments))/1000;
					
					this.slideTimer.fade({
						duration: d
					});
				}*/
			this.drawArc(0,endAngle,'rgba(255,255,255,0.5)');
			endAngle+=this.timerSegments;
			this.timerClock = setTimeout(function(){this.runTimer(endAngle)}.bind(this),this.timerSpeed);
		} else {			
			endAngle = 0;	
			this.stopTimer();
		}
	...
	drawArc: function(startAngle,endAngle,strokeStyle) {
		this.drawingArc = true;
		
		var ctx = this.timerCtx;	
		//console.log("drawArc: context is " + ctx );
		ctx.beginPath();		
